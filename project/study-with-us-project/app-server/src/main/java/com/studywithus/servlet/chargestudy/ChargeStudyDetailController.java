	...
	public void service(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (request.getAttribute("no") == null) {
				chargeStudyNo = Integer.parseInt(request.getParameter("no"));
				chargeStudyDao.updateCount(chargeStudyNo);

				// 다른 서블릿에서 왔다면
			} else {
				chargeStudyNo = (int) request.getAttribute("no");
			}

			Member member = (Member) request.getSession().getAttribute("loginUser");

			if (member != null) {
				result =  chargeStudyDao.checkLikesByMember(member.getNo(), chargeStudyNo);
			} else {
				result = 0;
			}

			Study chargeStudy = chargeStudyDao.findByNo(chargeStudyNo);
