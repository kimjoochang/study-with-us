...
CREATE TABLE mentee (
  ...
  register_date DATETIME NULL     DEFAULT now() COMMENT '등록일', -- 등록일
  remarks       TEXT     NULL     COMMENT '비고' -- 비고
)
COMMENT '멘티';
...
-- 멘티
ALTER TABLE mentee
  ADD CONSTRAINT PK_mentee -- 멘티 기본키
    PRIMARY KEY (
      member_no, -- 회원번호
      study_no   -- 스터디번호
    );
...
-- 유료스터디후기
ALTER TABLE review
  ADD CONSTRAINT FK_mentee_TO_review -- 멘티 -> 유료스터디후기
    FOREIGN KEY (
      member_no, -- 회원번호
      study_no   -- 스터디번호
    )
    REFERENCES mentee ( -- 멘티
      member_no, -- 회원번호
      study_no   -- 스터디번호
    );
...
-- 멘티
ALTER TABLE mentee
  ADD CONSTRAINT FK_study_TO_mentee -- 스터디 -> 멘티
    FOREIGN KEY (
      study_no -- 스터디번호
    )
    REFERENCES study ( -- 스터디
...
-- 멘티
ALTER TABLE mentee
  ADD CONSTRAINT FK_member_TO_mentee -- 회원 -> 멘티
    FOREIGN KEY (
      member_no -- 회원번호
    )
    REFERENCES member ( -- 회원
