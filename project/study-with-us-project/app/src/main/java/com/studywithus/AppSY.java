...
public class AppSY {
	...
	public AppSY() {
		...
		commandMap.put("/chargeStudy/payment", new ChargeStudyPaymentHandler(chargeStudyList, chargePaymentList,
				chargeApplicantList, participateChargeStudyMap));
		commandMap.put("/chargeStudy/paymentCancel", new ChargeStudyPaymentCancelHandler(chargeStudyList,
		...
		// commandMap.put("/chargeStudy/registerChargeStudy", new
		// RegisterChargeStudyDetailHandler(registerChargeStudyMap,
		// participateChargeStudyMap));
		...
		commandMap.put("/communityInfo/detail",
				new CommunityDetailHandler(communityInfoList, "/communityInfo/update", "/communityInfo/delete"));
		...
		commandMap.put("/communityTalk/detail",
				new CommunityDetailHandler(communityTalkList, "/communityTalk/update", "/communityTalk/delete"));
	...
	private <E> void loadObjects(String filepath, // 데이터를 읽어 올 파일 경로
			List<E> list, // 로딩한 데이터를 객체로 만든 후 저장할 목록
			Class<E> domainType // 생성할 객체의 타입 정보
			) {

		try (BufferedReader in = new BufferedReader(new FileReader(filepath, Charset.forName("UTF-8")))) {
	...
	/*
	 * 기존 App (2021-09-18 ver) 메뉴
	 * 
	 * package com.studywithus;
	 * 
	 * import static com.studywithus.menu.Menu.ACCESS_ADMIN; import static
	 * com.studywithus.menu.Menu.ACCESS_GENERAL; import static
	 * com.studywithus.menu.Menu.ACCESS_LEADER; import static
	 * com.studywithus.menu.Menu.ACCESS_LOGOUT; import static
	 * com.studywithus.menu.Menu.ACCESS_MENTOR; import java.io.BufferedReader;
	 * import java.io.BufferedWriter; import java.io.FileReader; import
	 * java.io.FileWriter; import java.io.PrintWriter; import
	 * java.lang.reflect.Type; import java.nio.charset.Charset; import
	 * java.util.ArrayList; import java.util.Collection; import java.util.HashMap;
	 * import java.util.LinkedList; import java.util.List; import
	 * com.google.gson.Gson; import com.google.gson.reflect.TypeToken; import
	 * com.studywithus.domain.Schedule; import com.studywithus.domain.Community;
	 * import com.studywithus.domain.Member; import
	 * com.studywithus.domain.MentorApplicationForm; import
	 * com.studywithus.domain.Payment; import com.studywithus.domain.Study; import
	 * com.studywithus.handler.Command; import
	 ...
	 * com.studywithus.handler.user.MyInfoHandler; import
	 * com.studywithus.handler.user.SignUpHandler; import com.studywithus.menu.Menu;
	 * import com.studywithus.menu.MenuGroup; import com.studywithus.util.Prompt;
	 * 
	 * public class AppOriginal { List<Member> memberList = new LinkedList<>();
	 * List<Member> freeApplicantList = new ArrayList<>(); List<Member>
	 * mentorApplicantList = new ArrayList<>(); List<Member> chargeApplicantList =
	 * new ArrayList<>();
	 * 
	 * List<String> mentorList = new ArrayList<>();
	 * 
	 * List<Study> registerFreeStudyList = new ArrayList<>(); List<Study>
	 * participateFreeStudyList = new ArrayList<>(); List<Study>
	 * registerChargeStudyList = new ArrayList<>(); List<Study>
	 * participateChargeStudyList = new ArrayList<>(); List<Study> freeInterestList
	 * = new ArrayList<>(); List<Study> chargeInterestList = new ArrayList<>();
	 * List<Study> freeStudyList = new ArrayList<>(); List<Study>
	 * freeApplicationList = new ArrayList<>(); List<Study> chargeStudyList = new
	 * ArrayList<>(); List<Study> chargeDeleteRequestList = new ArrayList<>();
	 * 
	 * List<MentorApplicationForm> mentorApplicationFormList = new ArrayList<>();
	 * 
	 * List<Payment> chargePaymentList = new ArrayList<>();
	 * 
	 * List<Community> communityInfoList = new ArrayList<>(); List<Community>
	 * communityQaList = new ArrayList<>(); List<Community> communityTalkList = new
	 * ArrayList<>();
	 * 
	 * List<Schedule> jobsCalendarList = new ArrayList<>(); List<Schedule>
	 * examCalendarList = new ArrayList<>();
	 * 
	 * HashMap<String, Command> commandMap = new HashMap<>(); HashMap<String,
	 * List<Study>> participateFreeStudyMap = new HashMap<>(); HashMap<String,
	 * List<Study>> participateChargeStudyMap = new HashMap<>(); HashMap<String,
	 * List<Study>> registerFreeStudyMap = new HashMap<>(); HashMap<String,
	 * List<Study>> registerChargeStudyMap = new HashMap<>();
	 * 
	 * class MenuItem extends Menu { String menuId;
	 * 
	 * public MenuItem(String title, String menuId) { super(title); this.menuId =
	 * menuId; }
	 * 
	 * public MenuItem(String title, int accessScope, String menuId) { super(title,
	 * accessScope); this.menuId = menuId; }
	 * 
	 * @Override public void execute() { Command command = commandMap.get(menuId);
	 * try { command.execute(new CommandRequest(commandMap)); } catch (Exception e)
	 ...
	 * }
	 * 
	 * public AppOriginal() { commandMap.put("/auth/login", new
	 * AuthLogInHandler(memberList)); commandMap.put("/auth/logout", new
	 * AuthLogOutHandler(memberList)); commandMap.put("/auth/signUp", new
	 * SignUpHandler(memberList)); commandMap.put("/auth/membershipwithdrawal", new
	 * MembershipWithdrawalHandler(memberList)); commandMap.put("/auth/userinfo",
	 * new MyInfoHandler(memberList));
	 * 
	 * commandMap.put("/freeInterest/list", new
	 * FreeInterestListHandler(freeInterestList));
	 * commandMap.put("/freeInterest/delete", new
	 * FreeInterestDeleteHandler(freeStudyList, freeInterestList));
	 ...
	 * 
	 * commandMap.put("/freeStudy/search", new
	 * FreeStudySearchHandler(freeStudyList)); commandMap.put("/freeStudy/add", new
	 * FreeStudyAddHandler(freeStudyList, registerFreeStudyMap));
	 * commandMap.put("/freeStudy/list", new FreeStudyListHandler(freeStudyList));
	 * commandMap.put("/freeStudy/detail", new FreeStudyDetailHandler(freeStudyList,
	 * freeApplicationList, freeInterestList)); commandMap.put("/freeStudy/update",
	 * new FreeStudyUpdateHandler(freeStudyList));
	 * commandMap.put("/freeStudy/delete", new
	 * FreeStudyDeleteHandler(freeStudyList));
	 * 
	 * commandMap.put("/chargeStudy/search", new
	 * ChargeStudySearchHandler(chargeStudyList));
	 ...
	 * ChargeStudyListHandler(chargeStudyList));
	 * commandMap.put("/chargeStudy/detail", new
	 * ChargeStudyDetailHandler(chargeStudyList, chargeInterestList,
	 * chargePaymentList, chargeApplicantList, participateChargeStudyMap));
	 * commandMap.put("/chargeStudy/update", new
	 * ChargeStudyUpdateHandler(chargeStudyList));
	 ...
	 * commandMap.put("/chargeStudy/payment", new
	 * ChargeStudyPaymentHandler(chargeStudyList, chargePaymentList,
	 * chargeApplicantList, participateChargeStudyMap));
	 * commandMap.put("/chargeStudy/interestAdd", new
	 * ChargeStudyInterestAddHandler(chargeStudyList, chargeInterestList));
	 * commandMap.put("/chargeStudy/InterestDelete", new
	 * ChargeStudyInterestDeleteHandler(chargeStudyList, chargeInterestList));
	 * 
	 * commandMap.put("/communityQa/add", new CommunityAddHandler(communityQaList));
	 * commandMap.put("/communityQa/list", new
	 * CommunityListHandler(communityQaList)); commandMap.put("/communityQa/detail",
	 * new CommunityDetailHandler(communityQaList));
	 * commandMap.put("/communityQa/update", new
	 * CommunityUpdateHandler(communityQaList));
	 * commandMap.put("/communityQa/delete", new
	 * CommunityDeleteHandler(communityQaList));
	 * commandMap.put("/communityQa/search", new
	 * CommunitySearchHandler(communityQaList));
	 * 
	 * commandMap.put("/communityInfo/add", new
	 * CommunityAddHandler(communityInfoList));
	 * commandMap.put("/communityInfo/list", new
	 * CommunityListHandler(communityInfoList));
	 * commandMap.put("/communityInfo/detail", new
	 * CommunityDetailHandler(communityInfoList));
	 * commandMap.put("/communityInfo/update", new
	 * CommunityUpdateHandler(communityInfoList));
	 * commandMap.put("/communityInfo/delete", new
	 * CommunityDeleteHandler(communityInfoList));
	 * commandMap.put("/communityInfo/search", new
	 * CommunitySearchHandler(communityInfoList));
	 * 
	 * commandMap.put("/communityTalk/add", new
	 * CommunityAddHandler(communityTalkList));
	 * commandMap.put("/communityTalk/list", new
	 * CommunityListHandler(communityTalkList));
	 * commandMap.put("/communityTalk/detail", new
	 * CommunityDetailHandler(communityTalkList));
	 * commandMap.put("/communityTalk/update", new
	 * CommunityUpdateHandler(communityTalkList));
	 * commandMap.put("/communityTalk/delete", new
	 * CommunityDeleteHandler(communityTalkList));
	 * commandMap.put("/communityTalk/search", new
	 * CommunitySearchHandler(communityTalkList));
	 * 
	 * commandMap.put("/jobsCalendar/add", new
	 * JobsScheduleAddHandler(jobsCalendarList));
	 * commandMap.put("/jobsCalendar/list", new
	 * JobsScheduleListHandler(jobsCalendarList));
	 * commandMap.put("/jobsCalendar/detail", new
	 * JobsScheduleDetailHandler(jobsCalendarList));
	 * commandMap.put("/jobsCalendar/update", new
	 * JobsScheduleUpdateHandler(jobsCalendarList));
	 * commandMap.put("/jobsCalendar/delete", new
	 * JobsScheduleDeleteHandler(jobsCalendarList));
	 * 
	 * commandMap.put("/examCalendar/add", new
	 * ExamScheduleAddHandler(examCalendarList));
	 * commandMap.put("/examCalendar/list", new
	 * ExamScheduleListHandler(examCalendarList));
	 * commandMap.put("/examCalendar/detail", new
	 * ExamScheduleDetailHandler(examCalendarList));
	 * commandMap.put("/examCalendar/update", new
	 * ExamScheduleUpdateHandler(examCalendarList));
	 * commandMap.put("/examCalendar/delete", new
	 * ExamScheduleDeleteHandler(examCalendarList)); }
	 * 
	 ...
	 * 
	 * saveObjects("member.json", memberList); saveObjects("freeInterest.json",
	 * freeInterestList); saveObjects("chargeInterest.json", chargeInterestList);
	 * saveObjects("freeStudy.json", freeStudyList); saveObjects("chargeStudy.json",
	 * chargeStudyList); saveObjects("communityQa.json", communityQaList);
	 * saveObjects("communityInfo.json", communityInfoList);
	 * saveObjects("communityTalk.json", communityTalkList);
	 * saveObjects("jobsCalendar.json", jobsCalendarList);
	 * saveObjects("examCalendar.json", examCalendarList); }
	 * 
	 * // JSON 형식으로 저장된 데이터를 읽어서 객체로 만든다. private <E> void loadObjects( String
	 * filepath, // 데이터를 읽어 올 파일 경로 List<E> list, // 로딩한 데이터를 객체로 만든 후 저장할 목록
	 * Class<E> domainType // 생성할 객체의 타입 정보 ) {
	 * 
	 ...
	 * strBuilder.append(str); }
	 * 
	 * // StringBuilder로 읽어 온 JSON 문자열을 객체로 바꾼다. Type type =
	 * TypeToken.getParameterized(Collection.class, domainType).getType();
	 * Collection<E> collection = new Gson().fromJson(strBuilder.toString(), type);
	 * 
	 * // JSON 데이터로 읽어온 목록을 파라미터로 받은 List 에 저장한다. list.addAll(collection);
	 * 
	 * System.out.printf("%s 데이터 로딩 완료!\n", filepath);
	 * 
	 * } catch (Exception e) { System.out.printf("%s 데이터 로딩 오류!\n", filepath); } }
	 * 
	 * // 객체를 JSON 형식으로 저장한다. private void saveObjects(String filepath, List<?>
	 * list) { try (PrintWriter out = new PrintWriter( new BufferedWriter( new
	 * FileWriter(filepath, Charset.forName("UTF-8"))))) {
	 * 
	 * out.print(new Gson().toJson(list));
	 * 
	 * System.out.printf("%s 데이터 출력 완료!\n", filepath);
	 * 
	 * } catch (Exception e) { System.out.printf("%s 데이터 출력 오류!\n", filepath);
	 * e.printStackTrace(); } }
	 * 
	 * Menu createMainMenu() { MenuGroup mainMenuGroup = new
	 * MenuGroup("STUDY WITH US"); mainMenuGroup.setPrevMenuTitle("종료");
	 * 
	 * mainMenuGroup.add(new MenuItem("로그인", ACCESS_LOGOUT, "/auth/login"));
	 * mainMenuGroup.add(new MenuItem("회원가입", ACCESS_LOGOUT, "/auth/signUp"));
	 * mainMenuGroup.add(new MenuItem("로그아웃", ACCESS_GENERAL | ACCESS_ADMIN,
	 * "/auth/logout")); mainMenuGroup.add(new MenuItem("회원 탈퇴", ACCESS_GENERAL,
	 * "/auth/membershipwithdrawal"));
	 * 
	 ...
	 * return mainMenuGroup; }
	 * 
	 * private Menu createMyPageMenu() { MenuGroup myPageMenu = new
	 * MenuGroup("마이 페이지", ACCESS_GENERAL);
	 * 
	 ...
	 * return myPageMenu; }
	 * 
	 * private Menu createAdminMenu() { MenuGroup adminMenu = new
	 * MenuGroup("관리자 페이지", ACCESS_ADMIN);
	 * 
	 ...
	 * return adminMenu; }
	 * 
	 * private Menu createMemberMenu() { MenuGroup memberMenu = new
	 * MenuGroup("회원 관리");
	 * 
	 ...
	 * return memberMenu; }
	 * 
	 * private Menu createMentorApplicantMenu() { MenuGroup mentorApplicantMenu =
	 * new MenuGroup("멘토 승인 관리");
	 * 
	 * mentorApplicantMenu.add(new MenuItem("조회", "/mentorApplicant/list"));
	 * mentorApplicantMenu.add(new MenuItem("상세보기", "/mentorApplicant/detail"));
	 * 
	 * return mentorApplicantMenu; }
	 * 
	 * private Menu createDeleteRequestStudyMenu() { MenuGroup deletedRequestMenu =
	 * new MenuGroup("삭제 요청 스터디 관리");
	 * 
	 * deletedRequestMenu.add(new MenuItem("조회", "/chargeStudy/deleteList"));
	 * 
	 * return deletedRequestMenu; }
	 * 
	 * private Menu createInterestMenu() { MenuGroup interestMenu = new
	 * MenuGroup("관심목록", ACCESS_GENERAL);
	 * 
	 * interestMenu.add(createFreeInterestMenu());
	 * interestMenu.add(createChargeInterestMenu());
	 * 
	 ...
	 * MenuGroup("무료 스터디 신청 내역", ACCESS_GENERAL);
	 * 
	 ...
	 * return freeStudyApplyMenu; }
	 * 
	 * private Menu createFreeStudyMenu() { MenuGroup freeStudyMenu = new
	 * MenuGroup("무료 스터디");
	 * 
	 * freeStudyMenu.add(new MenuItem("검색", "/freeStudy/search"));
	 * freeStudyMenu.add(new MenuItem("생성", ACCESS_GENERAL | ACCESS_LEADER,
	 * "/freeStudy/add")); freeStudyMenu.add(new MenuItem("조회", "/freeStudy/list"));
	 * freeStudyMenu.add(new MenuItem("상세보기", "/freeStudy/detail"));
	 * freeStudyMenu.add(new MenuItem("수정", ACCESS_LEADER, "/freeStudy/update"));
	 * freeStudyMenu.add(new MenuItem("삭제", ACCESS_LEADER | ACCESS_ADMIN,
	 * "/freeStudy/delete"));
	 ...
	 * return freeStudyMenu; }
	 * 
	 * private Menu createChargeStudyMenu() { MenuGroup chargeStudyMenu = new
	 * MenuGroup("유료 스터디");
	 * 
	 * chargeStudyMenu.add(new MenuItem("검색", "/chargeStudy/search"));
	 * chargeStudyMenu.add(new MenuItem("멘토 신청", ACCESS_GENERAL,
	 * "/mentorApplicant/add")); chargeStudyMenu.add(new MenuItem("생성",
	 * ACCESS_MENTOR, "/chargeStudy/add")); chargeStudyMenu.add(new MenuItem("조회",
	 * "/chargeStudy/list")); chargeStudyMenu.add(new MenuItem("상세보기",
	 * "/chargeStudy/detail")); chargeStudyMenu.add(new MenuItem("수정",
	 * ACCESS_MENTOR, "/chargeStudy/update")); chargeStudyMenu.add(new
	 * MenuItem("삭제 요청", ACCESS_MENTOR, "/chargeStudy/deleteRequest"));
	 * 
	 * return chargeStudyMenu; }
	 * 
	 * private Menu createCommunityMenu() { MenuGroup communityMenu = new
	 * MenuGroup("커뮤니티");
	 * 
	 * communityMenu.add(createCommunityInfoMenu());
	 * communityMenu.add(createCommunityQaMenu());
	 ...
	 * return communityMenu; }
	 * 
	 * private Menu createCommunityInfoMenu() { MenuGroup communityInfoMenu = new
	 * MenuGroup("정보");
	 * 
	 * communityInfoMenu.add(new MenuItem("검색", "/communityInfo/search"));
	 * communityInfoMenu.add(new MenuItem("생성", ACCESS_GENERAL,
	 * "/communityInfo/add")); communityInfoMenu.add(new MenuItem("조회",
	 * "/communityInfo/list")); communityInfoMenu.add(new MenuItem("상세보기",
	 * "/communityInfo/detail")); communityInfoMenu.add(new MenuItem("수정",
	 * ACCESS_GENERAL, "/communityInfo/update")); communityInfoMenu.add(new
	 * MenuItem("삭제", ACCESS_GENERAL | ACCESS_ADMIN, "/communityInfo/delete"));
	 * 
	 * return communityInfoMenu; }
	 * 
	 * private Menu createCommunityQaMenu() { MenuGroup communityQaMenu = new
	 * MenuGroup("질문");
	 * 
	 * communityQaMenu.add(new MenuItem("검색", "/communityQa/search"));
	 * communityQaMenu.add(new MenuItem("생성", ACCESS_GENERAL, "/communityQa/add"));
	 ...
	 * communityQaMenu.add(new MenuItem("상세보기", "/communityQa/detail"));
	 * communityQaMenu.add(new MenuItem("수정", ACCESS_GENERAL,
	 * "/communityQa/update")); communityQaMenu.add(new MenuItem("삭제",
	 * ACCESS_GENERAL | ACCESS_ADMIN, "/communityQa/delete"));
	 * 
	 * return communityQaMenu; }
	 * 
	 * private Menu createCommunityTalkMenu() { MenuGroup communityTalkMenu = new
	 * MenuGroup("스몰톡");
	 * 
	 * communityTalkMenu.add(new MenuItem("검색", "/communityTalk/search"));
	 * communityTalkMenu.add(new MenuItem("생성", ACCESS_GENERAL,
	 * "/communityTalk/add")); communityTalkMenu.add(new MenuItem("조회",
	 * "/communityTalk/list")); communityTalkMenu.add(new MenuItem("상세보기",
	 * "/communityTalk/detail")); communityTalkMenu.add(new MenuItem("수정",
	 * ACCESS_GENERAL, "/communityTalk/update")); communityTalkMenu.add(new
	 * MenuItem("삭제", ACCESS_GENERAL | ACCESS_ADMIN, "/communityTalk/delete"));
	 * 
	 * return communityTalkMenu; }
	 * 
	 * private Menu createCalendarMenu() { MenuGroup calendarMenu = new
	 * MenuGroup("캘린더");
	 * 
	 ...
	 * return calendarMenu; }
	 * 
	 * private Menu createJobsCalendarMenu() { MenuGroup jobsCalendarMenu = new
	 * MenuGroup("이달의 채용공고");
	 * 
	 ...
	 * jobsCalendarMenu.add(new MenuItem("수정", ACCESS_ADMIN,
	 * "/jobsCalendar/update")); jobsCalendarMenu.add(new MenuItem("삭제",
	 * ACCESS_ADMIN, "/jobsCalendar/delete"));
	 * 
	 * return jobsCalendarMenu; }
	 * 
	 * private Menu createExamCalendarMenu() { MenuGroup examCalendarMenu = new
	 * MenuGroup("이달의 시험일정");
	 * 
	 ...
	 * examCalendarMenu.add(new MenuItem("수정", ACCESS_ADMIN,
	 * "/examCalendar/update")); examCalendarMenu.add(new MenuItem("삭제",
	 * ACCESS_ADMIN, "/examCalendar/delete"));
	 * 
