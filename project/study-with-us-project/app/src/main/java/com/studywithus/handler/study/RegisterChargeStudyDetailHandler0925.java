...
public class RegisterChargeStudyDetailHandler0925 implements Command {

  // 각 멘토의 생성 유료 스터디 리스트
  HashMap<String, List<Study>> registerChargeStudyMap;
  HashMap<String, List<Study>> participateChargeStudyMap;

  public RegisterChargeStudyDetailHandler0925 (HashMap<String, List<Study>> registerChargeStudyMap,
      HashMap<String, List<Study>> participateChargeStudyMap) {
  ...
  private void studyMemberApproveHandler(Member chargeApplicant, Study chargeStudy) {
    ...
    if (participateChargeStudyMap.containsKey(chargeApplicant.getId())) {
      myParticipatedFreeStudy = participateChargeStudyMap.get(chargeApplicant.getId());
      myParticipatedFreeStudy.add(chargeStudy);
      participateChargeStudyMap.put(chargeApplicant.getId(), myParticipatedFreeStudy);
